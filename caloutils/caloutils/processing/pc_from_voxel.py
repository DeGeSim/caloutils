...
def convert_to_pc(voxel:Tensor,Einc:Tensor) -> Batch:
    ...
    showers = showers.reshape(-1,self.num_z, self.num_alpha, self.num_r)

    coords = torch.argwhere(showers > 0.) # get indices of non-zero values (shower_id, r, alpha, z)
    vals = showers[coords[:, 0], coords[:, 1], coords[:, 2], coords[:, 3]] # get non-zero values
    _, num_hits = torch.unique(coords[:, 0], return_counts=True) # get number of non-zero values per shower
    coords = coords[:, 1:] # remove shower_id from coords
    start_index = torch.zeros(num_hits.shape, dtype=torch.int64) # create start_index array
    start_index[1:] = num_hits.cumsum(0)[:-1] # calculate start_index
    x=torch.hstack((vals[:, None], coords))
    batch_idx = torch.arange(showers.shape[0]).repeat_interleave(num_hits) # create batch_idx array
