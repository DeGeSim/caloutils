...
from caloutils.utils.superbatch import SuperBatch
...
def test_batch_set_edges():
    ...
    batch.set_edges(torch.hstack(edges_list), batchidx_per_edge)
    compare(batch_truth, batch)
...
def test_batch_set_edge_attr():
    ...
    batch.set_edges(torch.hstack(edges_list), batchidx_per_edge)
    batch.set_edge_attr(torch.hstack(edge_attr_list), batchidx_per_edge)
    compare(batch_truth, batch)
...
def test_from_batch_list():
    ...
    batch_truth = Batch.from_data_list(batch_list)
    batch = SuperBatch.from_batch_list(
        [
